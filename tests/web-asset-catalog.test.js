...
test("the catalog resolves distinct Kenney furniture for every room object", async () => {
...
test("closed and open exits are separate Furniture Kit assets", async () => {
...
test("all vendored packs declare their official source and CC0 license", () => {
  for (const source of Object.values(ASSET_SOURCES)) {
    ...
    assert.match(source.sourceUrl, /^https:\/\/kenney\.nl\/assets\//);
  }
